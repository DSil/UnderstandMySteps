...
def simplify3(points, avg_speed):
    def find_angle(prev, curr, next):
        a = prev.distance_2d(curr)
        b = curr.distance_2d(next)
        c = prev.distance_2d(next)
        
        cos = min(1, (a*a + b*b - c*c) / (2 * a * b)) if (2 * a * b) != 0 else 0
        cos = max(-1, cos)
        angle = mod_math.acos(cos)
        angle = mod_math.degrees(angle)
    ...
    while i < len(points) - 1:
        ...
        curr = points[i]
        next = points[i+1]
        angle, distance = find_angle(prev, curr, next)
        if not (165 <= angle <= 195):
            speed = curr.speed_between(prev)
            time_dif = curr.time_difference(prev)
            if speed is not None and (speed > 10 * avg_speed or speed < 1.5):
                points.remove(curr)
            else:
                i += 1
        else:
